...
char *read_file(char *path) {
	...
	while (cur_length < target_length) {
		cur_length += fread(ret + cur_length, 1,
				    target_length - cur_length, file);
	}
	...
	if (**str == '"') {
		...
		if (**str == 0) {
			*str = start;
			...
			(*str)++;
			return lc_tr_c_value(
			    to_lc_string(start + 1, *str - start - 2));
		}
	}

	while (isgraph(**str) && !isspace(**str) && **str != '(' &&
	       **str != ')' && **str != '.' && **str != '\\') {
		(*str)++;
	}
...
static struct lc_tr *parse_lc_abs(char **str) {
	...
	if (take_char(str, '\\') && take_space(str) &&
	    (arg = parse_lc_ref(str)) && take_space(str) &&
	    take_char(str, '.') && take_space(str) && (body = parse_lc(str))) {
